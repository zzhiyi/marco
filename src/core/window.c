...
meta_window_update_struts (MetaWindow *window)
{
  ...
  if (meta_prop_get_cardinal_list (window->display,
                                   window->xwindow,
                                   window->display->atom__MATE_WM_STRUT_AREA,
                                   &struts, &nitems))
    {
      if (nitems != 4)
        {
          meta_verbose ("_MATE_WM_STRUT_AREA on %s has %d values instead of 4\n",
                        window->desc, nitems);
        }
      else
        {
    ...
    {
      meta_verbose ("No _MATE_WM_STRUT_AREA property for %s\n",
                    window->desc);
    }
